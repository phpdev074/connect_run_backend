...
export class GroupController {
  ...
  async requestJoin(@Param('id') id: string, @Req() req) {
    const data = await this.groupService.requestJoinGroup(req.user.id, id);
    return {
      statusCode: HttpStatus.OK,
  ...
  @ApiOperation({ summary: 'Get all join requests for a group (creator only)' })
  async getJoinRequests(@Param('id') id: string, @Req() req) {
    const data = await this.groupService.getJoinRequests(req.user.id, id);
    return {
      statusCode: HttpStatus.OK,
      success: true,
  ...
  @ApiOperation({ summary: 'Approve or reject a join request (creator only)' })
  ...
  async handleJoinRequest(
    ...
    let data;
    if (status === 'approved') {
      data = await this.groupService.approveJoinRequest(req.user.id, id, requestId);
    } else if (status === 'rejected') {
      data = await this.groupService.rejectJoinRequest(req.user.id, id, requestId);
    } else {
